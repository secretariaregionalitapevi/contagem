  ...
  if (queueStatusEl) {
    if (showSyncedStatus) {
      queueStatusEl.innerHTML = `
        ...
        SINCRONIZADO
      `;
      queueStatusEl.className = "queue-badge synced";
      ...
      queueStatusEl.innerHTML = `
        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="spin"><path d="M21 12a9 9 0 1 1-6.219-8.56"></path></svg>
        SINCRONIZANDO...
      `;
      queueStatusEl.className = "queue-badge syncing";
    } else {
      queueStatusEl.innerHTML = `
        <span style="font-size: 1.1em; line-height: 1;">${pendingCount}</span>
...
async function syncPending() {
  ...
  try {
    ...
    if (successCount > 0) {
      ...
      // Mostrar status de "SINCRONIZADO" temporariamente
      showSyncedStatus = true;
